...
function Dashboard() {
    ...
    useEffect(() => {
        fetch('/getTeamsList')
          ...
          .then((data) => setOptionsTeams(data))
          .catch((error) => console.error('Error fetching dropdown options:', error));
      }, []);
      useEffect(() => {
        fetch('/getPlayersList')
          .then((response) => response.json())
          .then((data) => setOptionsPlayes(data))
          .catch((error) => console.error('Error fetching dropdown options:', error));
      }, []);
      const [optionsPlayers, setOptionsPlayes] = useState([]);
      const [optionsTeams, setOptionsTeams] = useState([]);
      const [selectedValuePlayer, setSelectedValuePlayer] = useState('');
      ...
      const handlePlayerChange = (event) => {
          setSelectedValuePlayer(event.target.value);
       }; 

       const handleTeamChange = (event) => {
        setSelectedValueTeam(event.target.value);
      }; 
      ...
      formData.append('teamName', selectedValueTeam);
      formData.append('playerName', selectedValuePlayer);
      formData.append('contractWith', contractName);
      formData.append('uploadedBy', fullname);
      formData.append('contractType', 'Player');
      ...
      if (response.status===200) {
        ...
        setContractName('');
        setSelectedValuePlayer('');
        setSelectedFile(null);
        setSelectedValueTeam('');
        ...
        <ChakraProvider theme={theme}>
            ...
            <SimpleGrid columns={6} spacing={0} p='10'>
                ...
                <GridItem colSpan="4">
                    ...
                    <Box p='0' align="center" style={{ fontSize: "30px", fontWeight: "800" }}>Player</Box>
                </GridItem>
            </SimpleGrid>
                                ...
                                <CardBody p={10}>
                                    <Stack spacing={4}>
                                        ...
                                        <FormControl id="teamName">
                                            ...
                                            <Select placeholder='Select Team Name' value={selectedValueTeam} onChange={handleTeamChange}>
                                            {optionsTeams.map((optionsTeams) => (
                                                <option key={optionsTeams} value={optionsTeams}>
                                                {optionsTeams}
                                                </option>
                                            ))}
                                            </Select>
                                        </FormControl>
                                        <FormControl id="playerName">
                                            <FormLabel>Player Name</FormLabel>

                                            <Select placeholder='Select Player Name' value={selectedValuePlayer} onChange={handlePlayerChange}>
                                            {optionsPlayers.map((optionsPlayers) => (
                                                <option key={optionsPlayers} value={optionsPlayers}>
                                                {optionsPlayers}
                                                </option>
                                            ))}
                                            </Select>
